...
If you don't create a custom animation, all snapshot properties, including `width`, `height`, `originX`, `originY`, and `transformMatrix`, are animated by default with a duration of 500ms using the [`withTiming`](/docs/animations/withTiming/) animation.
...
<sharedTransitionTag is not working in FastImage>
